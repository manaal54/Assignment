...
var num1:number = 10;
var result:number = factorial(num1);
...
function removeNegativeNumbers(numbers: number[]): number[] {
  ...
  const numbersArray: number[] = [3, -5, 10, -2, 8, -7, 0, 4];
...
function celsiusToFahrenheit(celsiusTemperatures: number[]): number[] {
  ...
  // Example usage:
  const celsiusTemperatures: number[] = [0, 25, 100, -10];
  const fahrenheitTemperatures: number[] = celsiusToFahrenheit(celsiusTemperatures);
